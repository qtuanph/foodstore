<script lang="ts">
	...
	import { AddonsState } from './addons.svelte.js';

	const vm = new AddonsState();
...
<svelte:head><title>Quản lý Topping - Admin</title></svelte:head>

<div class="p-6">
	<div class="mb-6 flex items-center justify-between">
		<div>
			<h1 class="text-2xl font-bold text-gray-900">Quản lý Topping</h1>
			<p class="text-gray-600">Danh sách topping món ăn</p>
		</div>
		<Button variant="primary" onclick={() => vm.openCreateModal()}>+ Thêm topping</Button>
	</div>
...
<Modal
	open={vm.showModal}
	title={vm.editingItem ? 'Sửa topping' : 'Thêm topping'}
...
>
	...
	>
		<div>
			<label for="addonName" class="mb-2 block text-sm font-medium text-gray-700">Tên topping</label
			>
			<input
				id="addonName"
				type="text"
		...
		<div>
			<label for="addonPrice" class="mb-2 block text-sm font-medium text-gray-700"
				>Giá (0 = miễn phí)</label
			>
			<input
				id="addonPrice"
				type="number"
