<script lang="ts">
	...
	let accordionEl = $state<AccordionInterface | null>(null);

	onMount(() => {
		const el = document.getElementById(id);
		if (!el) return;

		// Build accordion items array for Flowbite API
		const accordionItems = items.map((item, i) => ({
			id: `${id}-body-${i}`,
			triggerEl: document.querySelector<HTMLElement>(`[data-accordion-target="#${id}-body-${i}"]`),
			targetEl: document.getElementById(`${id}-body-${i}`)
		}));

		accordionEl = new Accordion(el, accordionItems as any, {
			alwaysOpen,
			activeClasses: 'text-heading bg-neutral-secondary-soft',
			inactiveClasses: 'text-body'
		});
	});

	onDestroy(() => {
		accordionEl?.destroyAndRemoveInstance();
	});
</script>
